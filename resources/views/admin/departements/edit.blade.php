    ...
    <main class="p-4 lg:ml-64 h-auto pt-5">
        ...
        <div class="p-2 border-2 border-dashed rounded-lg border-gray-300 mb-4">
            <div class="bg-white w-full h-full flex flex-col rounded-lg">
                <form action="{{ route('departements.update', $departement->id) }}" method="post" class="p-4">
                    @csrf
                    @method('PUT')
                    <div class="grid gap-4">
                        ...
                        <div class="w-full">
                            ...
                            <x-text-input id="description" name="description" type="text" value="{{ old('description', $departement->description) }}" placeholder="description" />
                            <x-input-error :messages="$errors->get('description')" class="mt-2" />
                        </div>
                        {{-- status checkbox without blade component --}}
                        <div class="w-full">
                            <label class="relative inline-flex items-center mb-4 cursor-pointer">
                                <input type="checkbox" name="status" {{ $departement->status ? 'checked' : '' }} class="sr-only peer">
